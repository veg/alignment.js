...
class ScaffoldViewer extends Component {
  ...
  initialize(props) {
    if(props.fasta) {
      ...
      this.full_pixel_height = site_size*(this.sequence_data.number_of_sequences-1);
      ...
      this.scroll_broadcaster = new ScrollBroadcaster(
        ...
        { width: this.props.alignment_width, height: this.props.alignment_height },
  ...
  render() {
    ...
    const { alignment_width, alignment_height, scaffold_width } = this.props,
      gridTemplateColumns = `${this.label_width}px ${alignment_width}px ${scaffold_width}px`,
      container_style = {
        display: 'grid',
