...
function Tick(props) {
  const translateX = props.site_size / 2 + (props.site - 1) * props.site_size;
  return (
...
function SVGSiteAxis(props) {
  const sites = range(1, props.number_of_sites + 1, 2),
  ...
  return (
    ...
    >
      ...
      />
      {sites.map(site => (
        <Tick key={site} site={site} site_size={props.site_size} />
      ))}
    </g>
  );
}
